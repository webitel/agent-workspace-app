...
function mountNumpad({ attachTo }: { attachTo?: HTMLElement } = {}) {
...
describe('the-numpad', () => {
	...
	it('focuses the input when mounted', () => {
		const wrapper = mountNumpad({
	...
	it('returns focus to the input after a key is clicked', async () => {
		const wrapper = mountNumpad({
			attachTo: document.body,
		});
		const key = wrapper
			.findAll('button')
			.find((button) => button.text() === '5');

		(key?.element as HTMLButtonElement).focus();
